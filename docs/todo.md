...
**成果物**:
- [ ] `keywords.json`設定ファイル作成
- [ ] キーワードの`id`, `query`, `enabled`フィールド対応
- [ ] 複数キーワードのループ処理
- [ ] キーワードごとに検索実行
- [ ] 結果JSON内に`keyword`フィールド追加
- [ ] `enabled: false`のキーワードをスキップ
...
**成果物**:
- [ ] `index.html`: メインページ
- [ ] `app.js`: データ読み込みとレンダリング
- [ ] JSONファイル読み込み機能
- [ ] ツイート埋め込み表示
- [ ] Twitter埋め込みスクリプト統合
- [ ] シンプルなCSS（読みやすいレイアウト）
- [ ] ローカルサーバー起動（`npm run serve`など）
